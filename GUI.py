...
class AttendanceSystemGUI:
    ...
    def on_session_type_select(self, event):
        ...
        if selected_session == "Others":
            self.other_session_label = tk.Label(self.form_frame, text="Specify Session Type:", font=("Helvetica", 14), bg="#f4f4f4")
            self.other_session_label.grid(row=2, column=0, sticky=tk.W, pady=5)
            
            self.other_session_entry = tk.Entry(self.form_frame, font=("Helvetica", 12), width=30)
            self.other_session_entry.grid(row=2, column=1, pady=5)
            self.other_session_entry.grid_forget() 
            self.other_session_entry.grid(row=2, column=1, pady=5)  # Show the "Others" input field
        else:
            self.other_session_entry.grid_forget()  # Hide the "Others" input field
